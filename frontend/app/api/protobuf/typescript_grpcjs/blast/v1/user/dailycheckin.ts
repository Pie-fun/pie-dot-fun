...
export const GetDailyCheckinRequest: MessageFns<GetDailyCheckinRequest> = {
  encode(message: GetDailyCheckinRequest, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.user !== "") {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): GetDailyCheckinRequest {
...
export const ClaimDailyCheckinRequest: MessageFns<ClaimDailyCheckinRequest> = {
  encode(message: ClaimDailyCheckinRequest, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.user !== "") {
      writer.uint32(10).string(message.user);
    }
    return writer;
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): ClaimDailyCheckinRequest {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
...
export const DailyCheckin: MessageFns<DailyCheckin> = {
  encode(message: DailyCheckin, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.user !== "") {
      writer.uint32(10).string(message.user);
    }
    ...
    if (message.lastClaim !== undefined) {
      DailyCheckinClaim.encode(message.lastClaim, writer.uint32(42).fork()).join();
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): DailyCheckin {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
...
export const DailyCheckinClaim: MessageFns<DailyCheckinClaim> = {
  encode(message: DailyCheckinClaim, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.streak !== 0) {
    ...
    if (message.claimTime !== undefined) {
      Timestamp.encode(toTimestamp(message.claimTime), writer.uint32(26).fork()).join();
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): DailyCheckinClaim {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
...
export type DailyCheckinServiceService = typeof DailyCheckinServiceService;
export const DailyCheckinServiceService = {
  /** GetDailyCheckin will return the User's DailyCheckin */
  getDailyCheckin: {
    path: "/blast.v1.user.DailyCheckinService/GetDailyCheckin",
    requestStream: false,
    responseStream: false,
    requestSerialize: (value: GetDailyCheckinRequest) => Buffer.from(GetDailyCheckinRequest.encode(value).finish()),
    requestDeserialize: (value: Buffer) => GetDailyCheckinRequest.decode(value),
    responseSerialize: (value: DailyCheckin) => Buffer.from(DailyCheckin.encode(value).finish()),
    responseDeserialize: (value: Buffer) => DailyCheckin.decode(value),
  },
} as const;

export interface DailyCheckinServiceServer extends UntypedServiceImplementation {
  /** GetDailyCheckin will return the User's DailyCheckin */
  getDailyCheckin: handleUnaryCall<GetDailyCheckinRequest, DailyCheckin>;
}

export interface DailyCheckinServiceClient extends Client {
  /** GetDailyCheckin will return the User's DailyCheckin */
  getDailyCheckin(
    request: GetDailyCheckinRequest,
    callback: (error: ServiceError | null, response: DailyCheckin) => void,
  ): ClientUnaryCall;
  getDailyCheckin(
    request: GetDailyCheckinRequest,
    metadata: Metadata,
...
export const DailyCheckinServiceClient = makeGenericClientConstructor(
  DailyCheckinServiceService,
  "blast.v1.user.DailyCheckinService",
) as unknown as {
  new (address: string, credentials: ChannelCredentials, options?: Partial<ClientOptions>): DailyCheckinServiceClient;
  service: typeof DailyCheckinServiceService;
  serviceName: string;
};
