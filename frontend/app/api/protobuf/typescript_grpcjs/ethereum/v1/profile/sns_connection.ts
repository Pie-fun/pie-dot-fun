...
export const ExternalProfile: MessageFns<ExternalProfile> = {
  encode(message: ExternalProfile, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.name !== "") {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): ExternalProfile {
...
export const SNSConnection: MessageFns<SNSConnection> = {
  encode(message: SNSConnection, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.profile !== "") {
    ...
    if (message.externalProfile !== undefined) {
      ExternalProfile.encode(message.externalProfile, writer.uint32(34).fork()).join();
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): SNSConnection {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
