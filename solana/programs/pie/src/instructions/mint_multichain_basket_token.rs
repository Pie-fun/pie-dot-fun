...
use crate::{
    constant::USER_FUND, error::PieError, instructions::MintBasketTokenEvent, utils::Calculator, BasketConfig, UserFund, BASKET_CONFIG, BASKET_MINT
};

#[derive(Accounts)]
pub struct MintMultichainBasketTokenContext<'info> {
    #[account(mut)]
...
pub fn mint_multichain_basket_token(ctx: Context<MintMultichainBasketTokenContext>, basket_token_amount: u64) -> Result<()> {
    ...
    let mut components = basket_config.components.clone();

    // @TODO parse query message for multichain components
    let (multichain_components, other_components): (Vec<_>, Vec<_>) = components
            .into_iter()
        .partition(|component| component.mint.to_string().starts_with("1111111"));

    for token_config in other_components.iter() {
    ...
    msg!("other_components: {}", other_components.len());
    msg!("basket_token_amount: {}", basket_token_amount);
    for token_config in other_components.iter() {
            ...
            .find(|a| a.mint == token_config.mint)
        {
            ...
            msg!("token_config.quantity_in_sys_decimal: {}", token_config.quantity_in_sys_decimal);
            msg!("amount_to_deduct_in_raw_decimal: {}", amount_to_deduct_in_raw_decimal);
            asset.amount = asset
                .amount
